...
def end_game(result):
    global player_scores, bomb_count, bomb_row, bomb_column, bomb_touched, timer_id
...
def new_game():
    global player, moves, bomb_count, bomb_touched, timer_id, time_limit, time_left, ai_symbol, human_symbol
    ...
    time_limit = 5  # Resetting time_limit to 5 seconds
...
def next_turn(row, column):
    global player, bomb_touched, time_left
...
def undo_move():
    global player, bomb_count, bomb_row, bomb_column, bomb_touched, time_left
...
def start_game():
    global player, players, player_scores, moves, bomb_count, bomb_touched, ai_symbol, human_symbol

    player1 = entry1.get().strip()
    player2 = entry2.get().strip()

    if player1 == player2:
        label.config(text="Player names need to be different!")
        return
